...
class Ab_Testing_Software_Public {
	...
	function check_test_redirect () {
		...
		$testDays = $this->get_test_days($licenseCode);// check how many dates a test can be redirected
		
	//	die();
		
		$test_table = $wpdb->prefix . "abtesting_tests";
		$test_query = "SELECT * FROM $test_table WHERE `page_id` = '".$page_id."' AND `status` = 'running' ";
//		$test_query = $test_query . " and created_at>='$test_date'";
		$test_query = $test_query . " AND DATEDIFF(CURDATE(), DATE(created_at)) <= '$testDays'";
//		die();
...
//				wp_redirect( get_permalink( $page_id ) . '/?' . $_SERVER['QUERY_STRING'] );
				...
				$sql_pause = "update wp_9t3fji_abtesting_tests set status ='paused' where page_id = '$page_id' ";// reset all tests
